...
public class CheckTriggers extends Criterion {
	public CheckTriggers() {
		...
		boundedPredicate0.getBoundVariable().add(Variable.t);
		boundedPredicate0.getBoundVariable().add(Variable.c);
		boundedPredicate0.getPermittedNullVars().add(Variable.t);
		boundedPredicate0.setPredicate(new TriggerBelongToClass());
		brackets.getFormulas().add(boundedPredicate0);
		
		BoundedPredicate boundedPredicate1 = new BoundedPredicate();
		boundedPredicate1.getBoundVariable().add(Variable.l);
		boundedPredicate1.getBoundVariable().add(Variable.c);
		boundedPredicate1.getPermittedNullVars().add(Variable.l);
		boundedPredicate1.getPermittedNullVars().add(Variable.c);
		boundedPredicate1.setPredicate(new TransitionBelongToClass());
